...
export function AuditorTransactions() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-900">
      <Sidebar role={session?.role || 'AUDITOR'} onLogout={() => navigate('/staff/login')} /> {/* Pass actual role from session */}
      ...
      <div className="flex-1 overflow-x-hidden">
        ...
        <main className="p-6 max-w-5xl">
          <BankAccessPanel role={session?.role || 'AUDITOR'} onBankSelect={setActivePortal} /> {/* Pass actual role from session */}
        </main>
      </div>
